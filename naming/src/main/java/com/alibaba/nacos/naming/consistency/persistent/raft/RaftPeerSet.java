...
public class RaftPeerSet extends MemberChangeListener implements Closeable {
    ...
    public RaftPeer decideLeader(RaftPeer candidate) {
        ...
        for (RaftPeer peer : peers.values()) {
            ...
            // Meta- 候选者获取的票数不为0则保存当前的票数，票中的信息
        ...
        // Meta- peers.size() / 2 + 1 -----> 过半数的节点
        // Meta- 如果投票数超过半数节点，则设置当前候选者为leader
        // Meta- leader选举成功
        if (maxApproveCount >= majorityCount()) {
            ...
            if (!Objects.equals(leader, peer)) {
                // Meta- 第一次选举leader为空
                // Meta- 中途宕机leader与当前节点也不相同
                leader = peer;
                // Meta- 选举成功后发布leader选举完成事件
    ...
    public RaftPeer makeLeader(RaftPeer candidate) {
        ...
        for (final RaftPeer peer : peers.values()) {
            ...
            // Meta- 如果当前节点不是传入的candidate节点 且节点状态为leader
            if (!Objects.equals(peer, candidate) && peer.state == RaftPeer.State.LEADER) {
                try {
                    // Meta- 构建url
                    // Meta- URL： v1/us/raft/peer
